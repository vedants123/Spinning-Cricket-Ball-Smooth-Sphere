...
% Plot the original CD data
%figure; % Open a new figure window
plot(Time2, CZ2, 'b', 'DisplayName', '$C_{Z}$','LineWidth',2);
hold on;
...
% Plot the original CD data
%figure; % Open a new figure window
plot(Time3, CZ3, 'g', 'DisplayName', '$C_{Z}$','LineWidth',2);
hold on;
CZ4 = file4(:, 4)/16;

% Plot the original CD data
%figure; % Open a new figure window
plot(Time4, CZ4, 'm', 'DisplayName', '$C_{Z}$','LineWidth',2)
hold off;
...
ylabel('$\bar{C}_{Z}$', 'Interpreter', 'latex', 'FontSize', 15);
ax.XMinorTick='on';
ax.YMinorTick='on';
...
% Add grid and legend
% grid on;
legend('α = 0.0','α = 0.15','α = 0.35','α = 0.70','α = 1.0','Location','best') % Create the legend
 lgd.Interpreter = 'latex'; % Set LaTeX interpreter for the legend
exportgraphics(gcf, 'Cz_a0p0toa1.png', 'Resolution', 600);
% 
% 
